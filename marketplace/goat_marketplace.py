...
from marketplace.goat_response_parser import GoatResponseParser
...
class GoatMarketplace(MarketplaceAdapter):
    ...
    def __init__(
        self,
        client: GoatClientProtocol | None = None,
        parser: GoatResponseParser | None = None,
        ...
        self._client = client
        self._parser = parser or GoatResponseParser()
        self._matcher = matcher or ListingMatcher()
        self._comparator = PriceComparator()
        self.settings = settings or GoatSettings.from_env()
    ...
    def search(self, product: Product, query: str | None = None) -> MarketplaceSearchResult:
        ...
        for page in range(1, max_pages + 1):
            ...
            except GoatResponseError as exc:
                logger.warning("GOAT response error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except GoatParseError as exc:
                logger.warning("GOAT parse error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except GoatClientError as exc:
                logger.warning("GOAT client error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except Exception as exc:
                logger.warning(
                    "GOAT search failed on page %d: %s",
                    page,
                    exc.__class__.__name__,
                )
                result.status = SEARCH_ERROR
                result.error_message = "GOAT search failed"
                return result
            ...
            parsed = self._parser.parse(
                ...
                include_used=self.settings.include_used,
        ...
        logger.info(
            "GoatMarketplace search for %r: total=%d valid=%d rejected=%d warnings=%d",
            product.name,
            len(all_listings),
            len(ranked),
...
def create_goat_marketplace(
    client: GoatClientProtocol | None = None,
    settings: GoatSettings | None = None,
) -> GoatMarketplace:
    """Create GoatMarketplace from config or injected client."""
    if client is None:
    ...
    return GoatMarketplace(
        client=client,
        settings=settings or GoatSettings.from_env(),
    )
