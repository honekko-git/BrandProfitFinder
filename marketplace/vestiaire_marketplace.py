"""
Vestiaire Collective marketplace integration.
"""
...
from marketplace.vestiaire_exceptions import (
    VestiaireClientError,
    VestiaireConfigurationError,
    VestiaireParseError,
    VestiaireResponseError,
)
from marketplace.vestiaire_response_parser import VestiaireResponseParser
...
class VestiaireMarketplace(BaseMarketplace):
    """Vestiaire Collective marketplace (fixture/API-agnostic foundation)."""

    def __init__(
        self,
        client: VestiaireClientProtocol | None = None,
        parser: VestiaireResponseParser | None = None,
        matcher: ListingMatcher | None = None,
        settings: VestiaireSettings | None = None,
        selection_strategy: PriceSelectionStrategy = PriceSelectionStrategy.HIGHEST,
    ) -> None:
        self._client = client
        self._parser = parser or VestiaireResponseParser()
        self._matcher = matcher or ListingMatcher()
        self._comparator = PriceComparator()
        self.settings = settings or VestiaireSettings.from_env()
    ...
    def search(self, product: Product, query: str | None = None) -> MarketplaceSearchResult:
        """Search Vestiaire for listing candidates."""
        ...
        if self._client is None:
            message = (
                "Vestiaire Collective client is not configured. "
        ...
        for page in range(1, max_pages + 1):
            ...
            except VestiaireResponseError as exc:
                logger.warning("Vestiaire response error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except VestiaireParseError as exc:
                logger.warning("Vestiaire parse error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except VestiaireClientError as exc:
                logger.warning("Vestiaire client error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except Exception as exc:
                logger.warning("Vestiaire search failed on page %d: %s", page, exc.__class__.__name__)
                result.status = SEARCH_ERROR
                result.error_message = "Vestiaire search failed"
                return result
        ...
        logger.info(
            "VestiaireMarketplace search for %r: total=%d valid=%d rejected=%d warnings=%d",
            product.name,
            len(all_listings),
            len(ranked),
    ...
    def parse_listings(self, data: Any, source_query: str = "") -> list[MarketplaceListing]:
        ...
        Args:
            data: Parsed JSON payload or injected fixture for tests.
...
def create_vestiaire_marketplace(
    client: VestiaireClientProtocol | None = None,
    settings: VestiaireSettings | None = None,
) -> VestiaireMarketplace:
    """
    Create VestiaireMarketplace from config or injected client.

    Raises:
        VestiaireConfigurationError: When no client is provided.
    """
    if client is None:
        raise VestiaireConfigurationError(
            "Vestiaire Collective client is not configured. "
            "Use --demo-vestiaire for fixture demo mode."
        )
    return VestiaireMarketplace(client=client, settings=settings or VestiaireSettings.from_env())
