"""Search query generation for Rebag overseas acquisition."""
...
def build_rebag_search_queries(*, title: str, brand: str = "", category: str = "") -> list[str]:
    """Build up to three Rebag search queries from listing fields."""
    identity = extract_listing_identity(title=title, brand=brand, category=category)
    return build_rebag_identity_queries(identity=identity, title=title, brand=brand, category=category)


def build_rebag_identity_queries(
    *,
    ...
    """Build Rebag queries from normalized listing identity.
    ...
    queries.append(
        " ".join(
            part
            for part in [en_brand, cat.title() if cat else "", material.title() if material else "", color.title() if color else ""]
            if part
        )
    )
    queries.append(
        " ".join(part for part in [en_brand, family or cat.title() if cat else "", material.title() if material else ""] if part)
    )
...
def _title_fallback(title: str, *, brand: str) -> str:
    ...
    noise = {"rebag", "authenticated", "authentic", "excellent", "very", "good", "condition", "used", "investment", "piece"}
