"""
Fashionphile marketplace integration.
"""
...
from marketplace.fashionphile_exceptions import (
    FashionphileClientError,
    FashionphileConfigurationError,
    FashionphileParseError,
    FashionphileResponseError,
)
from marketplace.fashionphile_response_parser import FashionphileResponseParser
...
class FashionphileMarketplace(BaseMarketplace):
    """Fashionphile marketplace (fixture/API-agnostic foundation)."""

    def __init__(
        self,
        client: FashionphileClientProtocol | None = None,
        parser: FashionphileResponseParser | None = None,
        ...
        self._client = client
        self._parser = parser or FashionphileResponseParser()
        self._matcher = matcher or ListingMatcher()
        self._comparator = PriceComparator()
        self.settings = settings or FashionphileSettings.from_env()
    ...
    def search(self, product: Product, query: str | None = None) -> MarketplaceSearchResult:
        """Search Fashionphile for listing candidates."""
        ...
        if self._client is None:
            message = (
                "Fashionphile client is not configured. "
        ...
        for page in range(1, max_pages + 1):
            ...
            except FashionphileResponseError as exc:
                logger.warning("Fashionphile response error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except FashionphileParseError as exc:
                logger.warning("Fashionphile parse error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except FashionphileClientError as exc:
                logger.warning("Fashionphile client error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except Exception as exc:
                logger.warning(
                    "Fashionphile search failed on page %d: %s",
                    page,
                    exc.__class__.__name__,
                )
                result.status = SEARCH_ERROR
                result.error_message = "Fashionphile search failed"
                return result
        ...
        logger.info(
            "FashionphileMarketplace search for %r: total=%d valid=%d rejected=%d warnings=%d",
            product.name,
            len(all_listings),
            len(ranked),
...
def create_fashionphile_marketplace(
    client: FashionphileClientProtocol | None = None,
    settings: FashionphileSettings | None = None,
) -> FashionphileMarketplace:
    """
    Create FashionphileMarketplace from config or injected client.

    Raises:
        FashionphileConfigurationError: When no client is provided.
    """
    if client is None:
        raise FashionphileConfigurationError(
            "Fashionphile client is not configured. "
            "Use --demo-fashionphile for fixture demo mode."
        )
    return FashionphileMarketplace(
        client=client,
        settings=settings or FashionphileSettings.from_env(),
    )
