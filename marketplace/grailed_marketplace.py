"""
Grailed marketplace integration.
"""
...
from marketplace.grailed_exceptions import (
    GrailedClientError,
    GrailedConfigurationError,
    GrailedParseError,
    GrailedResponseError,
)
from marketplace.grailed_response_parser import GrailedResponseParser
...
class GrailedMarketplace(BaseMarketplace):
    """Grailed marketplace (fixture/API-agnostic foundation)."""

    def __init__(
        self,
        client: GrailedClientProtocol | None = None,
        parser: GrailedResponseParser | None = None,
        matcher: ListingMatcher | None = None,
        settings: GrailedSettings | None = None,
        selection_strategy: PriceSelectionStrategy = PriceSelectionStrategy.HIGHEST,
    ) -> None:
        self._client = client
        self._parser = parser or GrailedResponseParser()
        self._matcher = matcher or ListingMatcher()
        self._comparator = PriceComparator()
        self.settings = settings or GrailedSettings.from_env()
    ...
    def search(self, product: Product, query: str | None = None) -> MarketplaceSearchResult:
        """Search Grailed for listing candidates."""
        ...
        if self._client is None:
            message = (
                "Grailed client is not configured. "
        ...
        for page in range(1, max_pages + 1):
            ...
            except GrailedResponseError as exc:
                logger.warning("Grailed response error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except GrailedParseError as exc:
                logger.warning("Grailed parse error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except GrailedClientError as exc:
                logger.warning("Grailed client error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except Exception as exc:
                logger.warning(
                    "Grailed search failed on page %d: %s",
                    page,
                    exc.__class__.__name__,
                )
                result.status = SEARCH_ERROR
                result.error_message = "Grailed search failed"
                return result
            ...
            parsed = self._parser.parse(
                ...
                include_offer_enabled_only=self.settings.include_offer_enabled_only,
        ...
        logger.info(
            "GrailedMarketplace search for %r: total=%d valid=%d rejected=%d warnings=%d",
            product.name,
            len(all_listings),
            len(ranked),
...
def create_grailed_marketplace(
    client: GrailedClientProtocol | None = None,
    settings: GrailedSettings | None = None,
) -> GrailedMarketplace:
    """Create GrailedMarketplace from config or injected client."""
    if client is None:
        raise GrailedConfigurationError(
            "Grailed client is not configured. "
            "Use --demo-grailed for fixture demo mode."
        )
    return GrailedMarketplace(
        client=client,
        settings=settings or GrailedSettings.from_env(),
    )
