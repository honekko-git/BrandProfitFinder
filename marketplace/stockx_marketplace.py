...
from marketplace.stockx_response_parser import StockXResponseParser
...
class StockXMarketplace(MarketplaceAdapter):
    ...
    def __init__(
        self,
        client: StockXClientProtocol | None = None,
        parser: StockXResponseParser | None = None,
        matcher: ListingMatcher | None = None,
        settings: StockXSettings | None = None,
        selection_strategy: PriceSelectionStrategy = PriceSelectionStrategy.HIGHEST,
    ) -> None:
        self._client = client
        self._parser = parser or StockXResponseParser()
        self._matcher = matcher or ListingMatcher()
        self._comparator = PriceComparator()
        self.settings = settings or StockXSettings.from_env()
    ...
    def search(self, product: Product, query: str | None = None) -> MarketplaceSearchResult:
        ...
        if self._client is None:
            message = (
                "StockX client is not configured. "
        ...
        for page in range(1, max_pages + 1):
            ...
            except StockXResponseError as exc:
                logger.warning("StockX response error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except StockXParseError as exc:
                logger.warning("StockX parse error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except StockXClientError as exc:
                logger.warning("StockX client error on page %d: %s", page, exc)
                result.status = SEARCH_ERROR
                result.error_message = str(exc)
                return result
            except Exception as exc:
                logger.warning(
                    "StockX search failed on page %d: %s",
                    page,
                    exc.__class__.__name__,
                )
                result.status = SEARCH_ERROR
                result.error_message = "StockX search failed"
                return result
            ...
            parsed = self._parser.parse(
                ...
                include_inactive=self.settings.include_inactive,
                include_new=self.settings.include_new,
                include_preowned=self.settings.include_preowned,
                include_low_liquidity=self.settings.include_low_liquidity,
                require_known_lowest_ask=self.settings.require_known_lowest_ask,
                require_known_shipping=self.settings.require_known_shipping,
                require_known_fees=self.settings.require_known_fees,
                minimum_sales_last_30_days=self.settings.minimum_sales_last_30_days,
                minimum_asks_count=self.settings.minimum_asks_count,
                minimum_bids_count=self.settings.minimum_bids_count,
                maximum_volatility_rate=self.settings.maximum_volatility_rate,
                preferred_price_source=self.settings.preferred_price_source,
        ...
        logger.info(
            "StockXMarketplace search for %r: total=%d valid=%d rejected=%d warnings=%d",
            product.name,
            len(all_listings),
            len(ranked),
    ...
    def parse_listings(self, data, source_query: str = "") -> list[MarketplaceListing]:
...
def create_stockx_marketplace(
    client: StockXClientProtocol | None = None,
    settings: StockXSettings | None = None,
) -> StockXMarketplace:
    """Create StockXMarketplace from config or injected client."""
    if client is None:
        raise StockXConfigurationError(
            "StockX client is not configured. "
            "Use --demo-stockx for fixture demo mode."
        )
    return StockXMarketplace(
        client=client,
        settings=settings or StockXSettings.from_env(),
    )
