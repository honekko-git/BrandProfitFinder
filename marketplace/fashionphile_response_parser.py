"""
Fashionphile internal standard response parser.

Parses BrandProfitFinder internal Fashionphile fixture JSON (not an official API format).
"""
...
from config.constants import CURRENCY_JPY, MARKETPLACE_FASHIONPHILE
...
class FashionphileSaleStatus(str, Enum):
    """Listing sale status (distinct from item condition and inventory)."""
    ...
    @classmethod
    def from_value(cls, value: Any) -> "FashionphileSaleStatus":
        if value is None:
            return cls.UNKNOWN
...
class FashionphileInventoryStatus(str, Enum):
    ...
    @classmethod
    def from_value(cls, value: Any) -> "FashionphileInventoryStatus":
        if value is None:
            return cls.UNKNOWN
        text = str(value).strip().upper().replace("-", "_").replace(" ", "_")
        try:
...
@dataclass
class FashionphileParseResult:
    """Result of parsing a Fashionphile search payload."""
...
class FashionphileResponseParser:
    """Parse internal standard Fashionphile JSON into marketplace listings."""
    ...
    def parse(
        ...
        *,
    ...
    ) -> FashionphileParseResult:
        """
        Parse Fashionphile internal standard JSON into listings.

        Args:
            payload: BrandProfitFinder internal Fashionphile standard JSON.
            source_query: Query label for listings.
            include_sold: Include SOLD listings when True.
            include_reserved: Include RESERVED listings when True.
            include_unavailable: Include UNAVAILABLE/INACTIVE listings when True.
            include_discounted_only: Keep only discounted items when True.
            allow_unknown_currency: Allow non-JPY without conversion when True.
            default_currency: Expected currency when not specified.

        Returns:
            Parse result with listings, warnings, and counts.
        """
        result = FashionphileParseResult()
        items = payload.get("items")
        if items is None:
            return result
        if not isinstance(items, list):
            raise FashionphileParseError("Fashionphile items must be a list when present")
        ...
        for item in items:
            ...
            sale_status = FashionphileSaleStatus.from_value(item.get("sale_status"))
    ...
    def _parse_item(
        ...
        *,
        ...
        sale_status: FashionphileSaleStatus,
        inventory_status: FashionphileInventoryStatus,
        ...
        source_metadata: dict[str, Any] = {
            "source_marketplace": MARKETPLACE_FASHIONPHILE,
            ...
            "fashionphile_parse_warnings": "; ".join(warnings) if warnings else None,
        }
        ...
        return MarketplaceListing(
            marketplace_name=MARKETPLACE_FASHIONPHILE,
    ...
    def validate_payload(payload: dict[str, object]) -> None:
        ...
        if not isinstance(payload, dict):
            raise FashionphileResponseError("Fashionphile payload must be a JSON object")
        items = payload.get("items")
        if items is not None and not isinstance(items, list):
            raise FashionphileResponseError("Fashionphile items must be a list when present")
...
def _should_include_item(
    ...
    *,
    ...
    """Return True when item passes sale and inventory filters."""
    if sale_status == FashionphileSaleStatus.SOLD and not include_sold:
        return False
    if sale_status == FashionphileSaleStatus.RESERVED and not include_reserved:
        return False
    if sale_status in {
        FashionphileSaleStatus.INACTIVE,
        FashionphileSaleStatus.REMOVED,
    } and not include_unavailable:
        return False

    if inventory_status == FashionphileInventoryStatus.SOLD and not include_sold:
        return False
    if inventory_status == FashionphileInventoryStatus.RESERVED and not include_reserved:
        return False
    if inventory_status == FashionphileInventoryStatus.UNAVAILABLE and not include_unavailable:
        return False

    if sale_status in {
        FashionphileSaleStatus.ACTIVE,
        FashionphileSaleStatus.UNKNOWN,
    } and inventory_status in {
        FashionphileInventoryStatus.IN_STOCK,
        FashionphileInventoryStatus.LOW_STOCK,
        FashionphileInventoryStatus.UNKNOWN,
    }:
        return True

    if include_sold and (
        sale_status == FashionphileSaleStatus.SOLD
        or inventory_status == FashionphileInventoryStatus.SOLD
    ):
        return True
    if include_reserved and (
        sale_status == FashionphileSaleStatus.RESERVED
        or inventory_status == FashionphileInventoryStatus.RESERVED
    ):
        return True
    if include_unavailable and (
        sale_status in {FashionphileSaleStatus.INACTIVE, FashionphileSaleStatus.REMOVED}
        or inventory_status == FashionphileInventoryStatus.UNAVAILABLE
    ):
        return True
...
def _inventory_sale_warnings(
    ...
    warnings: list[str] = []
    if sale_status == FashionphileSaleStatus.ACTIVE and inventory_status in {
        FashionphileInventoryStatus.SOLD,
        FashionphileInventoryStatus.UNAVAILABLE,
    }:
        warnings.append(
            f"sale_status {sale_status.value} conflicts with inventory {inventory_status.value}"
        )
    if sale_status == FashionphileSaleStatus.SOLD and inventory_status in {
        FashionphileInventoryStatus.IN_STOCK,
        FashionphileInventoryStatus.LOW_STOCK,
    }:
        warnings.append(
...
def _parse_inventory_status(block: Any) -> FashionphileInventoryStatus:
    if not isinstance(block, dict):
        return FashionphileInventoryStatus.UNKNOWN
    return FashionphileInventoryStatus.from_value(block.get("status"))
...
def _parse_discount_block(block: Any) -> dict[str, Any]:
    ...
    result["discount_active"] = active is True
    ...
    if (
        ...
        expected = float(result["previous_price"]) - float(result["discount_amount"])
...
def _availability_from_status(
    ...
    if sale_status == FashionphileSaleStatus.SOLD or inventory_status == FashionphileInventoryStatus.SOLD:
        return "sold"
    if sale_status == FashionphileSaleStatus.RESERVED or inventory_status == FashionphileInventoryStatus.RESERVED:
        return "reserved"
    if inventory_status == FashionphileInventoryStatus.UNAVAILABLE:
        return "unavailable"
    if sale_status in {FashionphileSaleStatus.INACTIVE, FashionphileSaleStatus.REMOVED}:
        return "ended"
...
def _build_enricher_input(item: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    """Build enricher input from Fashionphile item, preprocessing condition text."""
    warnings: list[str] = []
    result = dict(item)
    condition_block = item.get("condition")
    if isinstance(condition_block, dict):
        raw = str(condition_block.get("raw") or "").strip()
        preprocessed, cond_warnings = preprocess_fashionphile_condition(raw)
        ...
        preprocessed, cond_warnings = preprocess_fashionphile_condition(condition_block)
        warnings.extend(cond_warnings)
        result["condition"] = preprocessed or condition_block
    return result, warnings


def preprocess_fashionphile_condition(raw: str | None) -> tuple[str, list[str]]:
    """
    Preprocess Fashionphile-specific condition strings before Phase 8 normalization.

    Does not upgrade ambiguous terms or conflate return-policy labels with condition.
    """
    warnings: list[str] = []
