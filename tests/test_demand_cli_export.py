"""Tests for demand opportunity ranking Excel export in discovery CLI."""
...
from profit_discovery.cli.demand_export import (
    DEMAND_OPPORTUNITY_RANKING_COLUMNS,
    SHEET_DEMAND_OPPORTUNITY_RANKING,
)
...
def test_demand_cli_export_adds_demand_opportunity_ranking_sheet(tmp_path: Path) -> None:
    ...
    sheet = workbook[SHEET_DEMAND_OPPORTUNITY_RANKING]
    headers = [cell.value for cell in sheet[1]]
    assert headers == list(DEMAND_OPPORTUNITY_RANKING_COLUMNS)
    assert sheet.max_row == len(result.ranked_demand_opportunities) + 1


def test_demand_cli_export_preserves_existing_profit_analysis_sheet(tmp_path: Path) -> None:
    result = run_discovery_command(
        DiscoveryCommandOptions(
