...
def test_fixture_return() -> None:
    ...
    assert client.search_items("nike")["total"] == 3


def test_records_query_page_filters() -> None:
    client = FakeStockXClient({"items": [], "total": 0})
...
def test_injected_exception() -> None:
    client = FakeStockXClient(error=RuntimeError("fail"))
    with pytest.raises(RuntimeError):
        client.search_items("x")


def test_multi_page() -> None:
    p1 = json.loads((FIXTURES / "stockx_search_page_1.json").read_text(encoding="utf-8"))
    p2 = json.loads((FIXTURES / "stockx_search_page_2.json").read_text(encoding="utf-8"))
    client = FakeStockXClient(pages={1: p1, 2: p2})
    assert client.search_items("nike", page=2)["page"] == 2
