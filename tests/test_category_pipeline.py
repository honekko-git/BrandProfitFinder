"""Pipeline tests for brand-category discovery."""
...
def test_brand_category_pipeline_runs_through_opportunity_ranking() -> None:
    brands = ["Chanel"]
    search_targets = build_discovery_search_targets(brands, category_priority="HIGH")
...
def test_category_catalog_and_resolver_generate_expected_chanel_wallet_query() -> None:
    catalog = CategoryCatalog.default()
    resolver = BrandCategoryResolver()
    categories = catalog.get_high_priority_categories()
    targets = resolver.resolve(["Chanel"], list(categories))
