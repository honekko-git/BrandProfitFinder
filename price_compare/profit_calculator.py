...
class ProfitCalculator:
    ...
    def _calculate_product(
        ...
        currency = product.currency.strip().upper() or "USD"
        exchange_rate = self._resolve_exchange_rate(product, currency)
        ...
        purchase_jpy = _convert_to_jpy(source_purchase, currency, exchange_rate)
        intl_shipping = self.config.international_shipping_jpy
        customs_base = purchase_jpy + intl_shipping
        customs_duty = _round_jpy(customs_base * self.config.customs_duty_rate)
        import_tax = _round_jpy((customs_base + customs_duty) * self.config.import_tax_rate)
        domestic_shipping = self.config.domestic_shipping_jpy
        other_costs = self.config.other_costs_jpy
        total_cost = purchase_jpy + intl_shipping + customs_duty + import_tax + domestic_shipping + other_costs
        marketplace_fee = _round_jpy(domestic_sale * self.config.marketplace_fee_rate)
        profit = domestic_sale - total_cost - marketplace_fee
        profit_margin = _safe_percentage(profit, domestic_sale)
        roi = _safe_percentage(profit, total_cost)
        ...
        return PriceResult(
            ...
            international_shipping_jpy=intl_shipping,
            customs_duty_jpy=customs_duty,
            import_tax_jpy=import_tax,
            domestic_shipping_jpy=domestic_shipping,
            marketplace_fee_jpy=marketplace_fee,
    ...
    def _resolve_exchange_rate(self, product: Product, currency: str) -> Decimal | None:
        if currency == "JPY":
            return Decimal("1")

        if product.exchange_rate and product.exchange_rate > 0:
            return _to_decimal(product.exchange_rate)

        configured = self.config.exchange_rates.get(currency)
        if configured is not None:
            return configured

        if currency in KNOWN_CURRENCIES:
            return _to_decimal(DEFAULT_EXCHANGE_RATE)
...
def _convert_to_jpy(amount: Decimal, currency: str, exchange_rate: Decimal) -> Decimal:
    if currency == "JPY":
        return _round_jpy(amount)
    return _round_jpy(amount * exchange_rate)


def _round_jpy(amount: Decimal) -> Decimal:
