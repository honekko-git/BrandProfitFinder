"""Verify Rebag overseas acquisition against Yahoo/Mercari domestic comps.
...
FIXTURE = ROOT / "tests" / "fixtures" / "browser_acquisition" / "rebag_search_results.html"
PRODUCT_COUNT = 20
...
def _domestic_title(identity, brand: str, category: str, *, overseas_title: str) -> str:
    ...
    overseas = " ".join(
        ...
        if token.lower() not in {"rebag", "authenticated", "condition", "excellent", "very", "good", "fair"}
    )[:90]
...
def main() -> int:
    listings = parse_rebag_html(FIXTURE.read_text(encoding="utf-8"))[:PRODUCT_COUNT]
    ...
    for index, item in enumerate(listings, start=1):
        ...
        candidate = BatchProfitCandidate(
            candidate_id=f"rebag-{index:02d}",
            title=item.title,
            brand=brand,
            category=category,
            detected_subtype="",
        ...
        weak = " ".join(
            ...
            for part in [
                ...
                CATEGORY_JA.get(identity.category, "バッグ"),
            ]
            if part
        )
        yahoo_title = strong if index % 2 else weak
        mercari_title = weak if index % 2 else strong
        yahoo_price = max(10000, int(jpy) + 20000 + index * 500)
        mercari_price = max(10000, int(jpy) + 18000 + index * 500)
        sold = _yahoo_sold_html(yahoo_title, yahoo_price, f"yr{index:04d}")
        opened = _yahoo_open_html(yahoo_title, yahoo_price + 1500, f"yo{index:04d}")
        ...
        base_rows.append(
            {
                "rebag_title": item.title,
                "rebag_url": item.url,
                "rebag_usd": str(item.price),
    ...
    profit_ok = 0
    detailed = []
    for item, base in zip(run.results, base_rows, strict=False):
        row = {
            **base,
            "yahoo_queries": list(item.yahoo_queries),
            "accepted_comparable_count": item.accepted_comparable_count,
            "rejected_comparable_count": item.rejected_sample_count,
            "selected_marketplace": item.yahoo_marketplace,
            "selected_title": item.yahoo_best_title,
            "selected_price_jpy": item.yahoo_best_price_jpy,
            "selected_url": item.yahoo_best_url,
            ...
            "acquisition_source": item.candidate.purchase_source,
            "purchase_url": item.candidate.purchase_url,
        }
        detailed.append(row)
        if item.yahoo_marketplace == "Yahoo Auctions":
            ...
            no_comparable += 1
        if item.yahoo_best_score >= 70:
            successful += 1
        elif item.yahoo_best_score >= 45:
            near_miss += 1
        if item.net_estimated_profit is not None or item.gross_estimated_profit != 0:
            profit_ok += 1

    report = {
        "verification_mode": "fixture",
        "note": (
            "Fixture-based end-to-end verification using saved Rebag HTML plus "
            "identity-matched Yahoo/Mercari fixtures. Not live results."
        ),
        "product_count": len(candidates),
        "valid_rebag_acquisition_candidates": len(candidates),
    ...
    out = ROOT / "output" / "rebag_integration_verification.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    summary = {k: report[k] for k in (
        "verification_mode",
        "product_count",
        "valid_rebag_acquisition_candidates",
        "successful_matches",
    ...
    ok = (
        ...
        and report["valid_rebag_acquisition_candidates"] >= 20
        and report["url_failures"] == 0
        and report["yahoo_selected_as_best"] + report["mercari_selected_as_best"] >= 10
        and all(row["acquisition_source"] == "Rebag" for row in detailed)
        and all(row["purchase_url"].startswith("https://www.rebag.com/products/") for row in detailed)
    )
    return 0 if ok else 1


def _optional_live_probe() -> dict:
    """Optional live Rebag search smoke; never represented as fixture success."""
    try:
        from marketplace.browser_acquisition.rebag_acquirer import RebagAcquirer

        result = RebagAcquirer(purchase_limit=5).search_keyword("Chanel wallet")
        return {
            "attempted": True,
        ...
        return {
            ...
            "error": str(exc),
            "note": (
                "Live Rebag search currently blocked (CAPTCHA/empty DOM). "
                "Use saved HTML upload. Fixture verification remains separate."
            ),
        }
